<?php
/**
 * Syfte: Visar och hantera HELA gästboken (alla inlägg) – skapa, redigera, ta bort.
 * Säkerhet: Kräver inloggning? och bara användrna själv kan redigera deras gästbok inlägg
 */
session_start();
date_default_timezone_set('Europe/Stockholm'); // sverige's tidszon
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  // Add new entry (TODO: validera CSRF när token införs)
  ...
  // Reload entries: efter eventuell skrivning för att få färsk lista
  $entries = [];
  if (file_exists($filename)) {
      $json = file_get_contents($filename);
      $entries = json_decode($json, true) ?? [];
  ...
  // Delete entry (ägare eller admin)
  ...
  // Om endast edit_entry satt: sidan renderar edit-formuläret utan redirect.
}
$entries = [];
if (file_exists($filename)) {
    $json = file_get_contents($filename);
...
<div class="guestbook-wrap">
  ...
  <div class="guestbook-entries">
    ...
    foreach (array_reverse($entries) as $displayIndex => $entry) {
        ...
        // If editing this entry, show edit form (bsrs använderna eller admin)
        if (
